...
const STUN_SERVER: &str = "stun:google.de:3478";
const SELF_STUN_SERVER: &str = "stun:192.168.31.37:3478";
...
fn get_config() -> RTCConfiguration{
    return RTCConfiguration {
        ice_servers: vec![
            ...
            RTCIceServer {
                urls: vec![SELF_STUN_SERVER.to_owned()],
                ..Default::default()
            }
        ],
...
pub async fn simple_stun() -> anyhow::Result<()> {
...
async fn answer() -> anyhow::Result<Arc<RTCPeerConnection>> {
    ...
    let peer_connection = Arc::new(api.new_peer_connection(get_config()).await?);
