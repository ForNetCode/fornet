...
pub fn create_async_tun(guid:&str, mtu:u32, address:&[AllowedIP], driver_path: String) -> anyhow::Result<(ReadPart, WritePart, String)> {
    let (read_part, write_part, device) = fortun_cli::create_async_tun(&GUID::from(guid), "ForTun", driver_path)?;
